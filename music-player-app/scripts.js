document.addEventListener("DOMContentLoaded", () => {
  ...
  const deleteBackgroundIcon = document.getElementById(
...
function saveSong(song) {
    ...
    const objectStore = transaction.objectStore('songs');
    objectStore.put(song); // Use put instead of add to prevent duplicates
}
  ...
  function updatePlaylist(playlistId) {
    ...
    songs[playlistId].forEach((song, index) => {
      ...
      songElement.innerHTML = `
                <span class="songBlock">${
                  isPlaying && currentSongIndex === index
                    ? '<img src="vinyl.png" class="rotating">'
                    : ""
                } ${song.name}</span>
    ...
    document.getElementById(`track-count-${playlistId}`).textContent =
      songs[playlistId].length;
  ...
  playlistHeaders.forEach((header) => {
    header.addEventListener("click", () => {
      ...
      if (playlistId !== currentPlaylist) {
        document
          .getElementById(`playlist-${currentPlaylist}`)
  ...
  // Save entire playlist to IndexedDB
  function savePlaylist() {
    ...
    const transaction = db.transaction(["songs"], "readwrite");
    const objectStore = transaction.objectStore("songs");
    objectStore.clear(); // Clear all songs before updating

    [...songs[1], ...songs[2]].forEach((song) => {
      objectStore.put(song); // Save all songs from both playlists
    });
  }
